...
const params = {
  ...
  condition: {
    ...
    topics: [
      "0x4750cdedb10bb169e6443b06cde799d98b25e9c9b868fbfae35c19efce8d70a3", //swap performed
    ],
  },
};
    ...
    socket.on("connect", () => {
      ...
      socket.on("subscription_event", async (message) => {
        ...
        try {
          ...
          const dataBytes = event.messages[0].data;
          const types = ["uint256", "string"];

          const coder = AbiCoder.defaultAbiCoder();
          const decoded = coder.decode(types, dataBytes);

          const amount = BigInt(decoded[0]);
          const amountNumber = Number(amount);

          const tokenName = decoded[1];

          console.log("🧩 Extracted amount:", amountNumber);
          console.log("🧩 Extracted tokenName:", tokenName);
